...
$(function () {
  ...
  $.fn.jQBanner = function (defaults) {
    ...
    config.objScr = "#" + this.attr("id") + " .BannerBig";
    config.objBtn = "#" + this.attr("id") + " .BannerSmall";
  ...
  function jsBanInit(config) {
    ...
    $(config.objScr).css({
      position: "relative",
      height: config.nHeight + "px",
      width: config.nWidth + "px",
      overflow: "hidden",
    });
    ...
    /* 버튼 순번 설정 */
    $(config.objBtn + " " + config.isBtnType).each(function (i) {
      $(this).attr("btnno", i);
  ...
  function jsBanAction(config) {
    ...
    if (config.nOrderNoBtn >= 0) {
      ...
      nSpeed =
        config.isActType == "fade" || config.isActType == "page" || config.isActType == "none" ? config.nSpeedFade : 0;
    } else {
    ...
    if (config.isBtnAct == "mouseover") {
      ...
      $(config.objBtn + " " + config.isBtnType).mouseover(function () {
        ...
        if (config.isActType == "page") {
          ...
          if (nImgNo == 1) {
            config.nOrderNoBtn =
              config.nOrderNo + 1 == config.nCount ? (config.nOrderNo + 1) % config.nCount : config.nOrderNo + 1;
          }
...
$(function () {
  $("#movepopup").jQBanner({
    //롤링을 할 영역의 ID 값
    nWidth: 634, //영역의 width
    nHeight: 338, //영역의 height
    nCount: 3, //돌아갈 이미지 개수
